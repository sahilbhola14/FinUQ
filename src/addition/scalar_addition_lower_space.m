% Performs Scalar Addition when the numbers are sampled in the lower space 
...
%% Summation
numbers = zeros(num_mcmc_samples, 2);
for ii = 1:num_mcmc_samples
    ...
    numbers(ii, :)      = convert_precision(numbers_samples, 'double');
    numbers_samples_dp  = convert_precision(numbers_samples, 'double');
    % True Summation
    summation_dp        = compute_summation(numbers_samples_dp);
    % Lower Precisin Summation
    summation_lp        = compute_summation(numbers_samples);
    % Model Summation
...
%% Figure: Abs Error Matrix
figure()
scatter(numbers(:, 1), numbers(:, 2), 50, abs_error_true, 'filled', 'MarkerEdgeColor', 'k')
colorbar
colormap parula
set(gca, 'FontSize', 20, 'TickLabelInterpreter', 'latex')
xlabel('$\hat{a}$', 'Interpreter', 'latex', 'FontSize', 20)
ylabel('$\hat{b}$', 'Interpreter', 'latex', 'FontSize', 20)
grid on
box on
% assert(1==2)
...
figure()
subplot(1, 2, 1)
histogram(abs_error_true, 'Normalization', 'pdf', 'DisplayName', 'True', 'NumBins', 50)
hold on
xline(urd, 'LineWidth', 5, 'Color', 'k', 'LineStyle', '--', 'DisplayName', '$u$')
hold off
set(gca, 'FontSize', 20, 'TickLabelInterpreter', 'latex')
%xlabel('$\xi = \vert (\hat{a} \oplus \hat{b}) - (\hat{a} + \hat{b}) \vert$', 'Interpreter', 'latex', 'FontSize', 20)
xlabel('$\xi = \vert fl(a + b) - (a+b) \vert$', 'Interpreter', 'latex', 'FontSize', 20)
...
hold off
set(gca, 'FontSize', 20, 'TickLabelInterpreter', 'latex')
%xlabel('$\xi = \vert \frac{(\hat{a} \oplus \hat{b}) - (\hat{a} + \hat{b})}{(\hat{a} + \hat{b})} \vert$', 'Interpreter', 'latex', 'FontSize', 20)
xlabel('$\xi = \vert \frac{fl(a + b) - (a+b)}{(a+b)} \vert$', 'Interpreter', 'latex', 'FontSize', 20)
ylabel('$f_{\Xi}(\xi)$', 'Interpreter', 'latex', 'FontSize', 20)
...
save(['cdf_scalar_addition_k_', num2str(k), '_num_samples_', num2str(num_mcmc_samples), '_space_', convertStringsToChars(sample_space), '.mat'], 'F', 'X', 'F_model', 'X_model')
...
title(['Relative Error CDF ', '$(a, b \sim \mathcal{U}[2^{', num2str(k), '}, 2^{', num2str(k+1), '}])$'], 'Interpreter', 'latex', 'FontSize', 20)
lg = legend(); set(lg, 'Interpreter', 'latex', 'FontSize', 20)
grid on
box on
...
function summation = compute_summation(numbers)
