classdef DeltaOverload
    ...
    methods
        ...
        function obj = check_config(obj, config)
            ...
            if config("analysis") == "probabilistic"
                assert (isKey(config, "rel_error_dist"), "Invalid config")
                assert (config("rel_error_dist") == "uniform" || config("rel_error_dist") == "model", "Invalid config")
            end
        end
...
function samples = sample_probabilistic(config, num_samples)
    ...
    if config("rel_error_dist") == "uniform"
        vals = { num_samples, 'uniform', config("precision") };
        args = containers.Map(keys, vals);
    elseif config("rel_error_dist") == "model"
