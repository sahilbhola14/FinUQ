...
for ii = 1:num_mcmc_samples
    ...
    true_prod = samples(1) * samples(2);
    prod_float = double(single(samples(1)) * single(samples(2)));
    
    % Relative Error
    abs_rel_error_true(ii) = compute_abs_rel_error(true_prod, prod_float);
...
function abs_rel_error = compute_abs_rel_error_model(samples, precision, representation_error)
    ...
    if representation_error
        ...
        model_prod = (val_1*(1+rel_samples(1)) * val_2*(1+rel_samples(2)))*(1+rel_samples(3));
    else
        rel_samples = get_rel_error_samples(1, precision);
        model_prod = (val_1* val_2)*(1+rel_samples(1));
    end
    true_prod = val_1 * val_2;
    abs_rel_error = abs(true_prod - model_prod) / abs(true_prod);
end
...
function bound = compute_deterministic_f_error_bound(samples, precision, representation_error)
    ...
    true_prod = val_1 * val_2;
    if representation_error
        gamma_three = compute_deterministic_gamma(3, precision);
        bound = gamma_three*(abs(val_1) * abs(val_2)) / abs(true_prod);
    else
        gamma_one = compute_deterministic_gamma(1, precision);
        bound = gamma_one*(abs(val_1) * abs(val_2)) / abs(true_prod);
    end
end
...
function bound = compute_probabilistic_f_error_bound(samples, precision, representation_error, lambda_val)
    ...
    true_prod = val_1 * val_2;
    if representation_error
        gamma_three = compute_probabilistic_gamma(3, precision, lambda_val);
        bound = gamma_three.*(abs(val_1) * abs(val_2)) / abs(true_prod);
    else
...
function e_bwd = compute_true_bwd_error(samples)
    ...
    true_prod = val_1 * val_2;
    prod_float = double(single(samples(1)) * single(samples(2)));
    e_bwd = abs(true_prod - prod_float) / (abs(val_1) * abs(val_2));
end
...
function e_bwd = compute_modeled_bwd_error(samples, precision, representation_error)
    ...
    e_bwd = abs(true_prod - model_prod) / (abs(val_1) * abs(val_2));
end

function bound = compute_deterministic_bwd_error_bound(precision, representation_error)
    if representation_error
        bound = compute_deterministic_gamma(3, precision);
    else
...
function bound = compute_probabilistic_bwd_error_bound(precision, representation_error, lambda_val)
    if representation_error
        bound = compute_probabilistic_gamma(3, precision, lambda_val);
    else 
